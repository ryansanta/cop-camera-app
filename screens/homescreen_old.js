...
import { createFile, listFile, createFolder } from '../components/google/gHelpers';
...
export default function HomeScreen({ navigation, route, item }) {
  ...
  async function navAuth() {
      let check;
      let gInit = GDrive.isInitialized();
      console.log('gInit:', gInit);
      if (gInit === true){
        if (await getKeyInfo(siteID) === null) {
          // console.log('getKeyInfo null:', await getKeyInfo(siteID));
          ...
          // console.log('getKeyInfo else:', await getKeyInfo(siteID));
          let check = await checkParent();
          let parentFolder = check;
          let item = await getMulti(siteID);
          navigation.navigate('COP Photo List', { siteid: siteID, parentfolder: parentFolder, item: item });
        ...
        Alert.alert('Google Drive', 'You must login to Google Drive!', [
  ...
  return (
            ...
            <TextInput value={siteID} style={styles.textarea} placeholder='e.g. 1248-VZW' onChangeText={(thesiteid) => setSiteID(thesiteid)} textAlign={'center'} />
              <GAuth />
              <TouchableOpacity style={styles.button} onPress={navAuth} hitSlop={{top: 20, bottom: 20, left: 50, right: 50}}>
